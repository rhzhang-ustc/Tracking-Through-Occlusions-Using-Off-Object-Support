...
log_dir = 'test_logs'
model_name_suffix = 'conv_iter'
ckpt_dir = 'checkpoints_conv_iter'
num_worker = 12
iters = 6
...
def run_model(model, encoder, rgbs, trajs, target_traj, target_traj_e, valids, criterion, sw):
    ...
    '''
    trajs_encoding: encode the trajectories we have
    '''
    ...
    '''
    target_e: current estimation of target's trajectory
    '''

    relative_target_e = target_traj_e - start_target_pos     # B, S, 1, 2
    target_e_encoding = utils.misc.get_2d_embedding(relative_target_e.reshape(B * S, 1, 2), num_band)
    target_e_encoding = target_e_encoding.reshape(B, S, 1, -1).repeat(1, 1, N0, 1)    # B, S, 1, 66
    ...
    motion_encoding = utils.misc.get_2d_embedding(relative_motion.reshape(B * S, N0, 2), num_band)
    motion_encoding = motion_encoding.reshape(B, S, N0, -1)  # B, S, N0, 66
    ...
    input_matrix = torch.concat([trajs_encoding,
                                 ...
                                 target_traj_feature,
                                 target_e_encoding,
                                 valids.unsqueeze(-1)], axis=-1)    # B, S, N0, 405

    input_matrix = input_matrix.permute(0, 2, 3, 1).reshape(B*N0, -1, S)   # B*N0, 405, S
...
def train():
    ...
    model = ResNet1D(in_channels=3 * (2 * num_band + 2) + 2 * feature_map_dim + 1,
                     base_filters=512,
                     kernel_size=5,
                     stride=1,
                     n_block=10,
    ...
    while global_step < max_iters:
        ...
        target_trajs_e = target_traj[:, 0:1, :, :].repeat(1, S, 1, 1).detach()   # B, S, 1, 2
        total_loss = torch.tensor(0.0, requires_grad=True).to(device)

        for i in range(iters):

            loss,  avg_error, pred_traj = run_model(model, encoder,
                                                    rgbs, trajs_e, target_traj, target_trajs_e,
                                                    vis_e, criterion, sw_t)

            total_loss = total_loss + loss
            target_trajs_e = pred_traj.clone().detach()
