...
log_dir = 'test_logs'
model_name_suffix = 'conv_16'
ckpt_dir = 'checkpoints_16'
num_worker = 12
...
def run_model(model, encoder, rgbs, trajs, target_traj, valids, criterion, sw):
    ...
    trajs_encoding = utils.misc.get_3d_embedding(torch.concat([relative_pos, t_pos], dim=-1).reshape(B * S, N0, 3), num_band)
    trajs_encoding = trajs_encoding.reshape(B, S, N0, -1)  # B, S, N0, 99
...
def train():
    ...
    model = ResNet1D(in_channels=(2 * num_band + 2) + (3 * num_band + 3) + 2 * feature_map_dim + 1,
                     base_filters=512,
                     kernel_size=5,
                     stride=1,
                     n_block=10,
    ...
    while global_step < max_iters:
        ...
        rgbs_flip = rgbs.flip(1)
        trajs_e1, vis_e1 = run_pips(pips, rgbs, N, sw_t)  # N-2
        trajs_e2, vis_e2 = run_pips(pips, rgbs_flip, N, sw_t)
        ...
        rgbs_extend = torch.concat([rgbs, rgbs_flip], dim=1)
        trajs_e_extend = torch.concat([trajs_e1, trajs_e2], dim=1)
        target_traj_extend = torch.concat([target_traj, target_traj.flip(1)], dim=1)
        vis_e_extend = torch.concat([vis_e1, vis_e2], dim=1)

        '''
        train
        '''

        total_loss, avg_error, _ = run_model(model, encoder, rgbs_extend, trajs_e_extend,
                                             target_traj_extend, vis_e_extend, criterion, sw_t)
